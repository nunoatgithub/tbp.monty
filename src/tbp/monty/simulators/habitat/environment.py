...
class HabitatEnvironment(EmbodiedEnvironment):
    ...
    def __init__(
        ...
        # Create IPC wrapper configuration
        ipc_config = HabitatSimConfig(
            agents=self._agents,
            scene_id=scene_id,
            seed=seed,
            data_path=data_path,
        ...
        # Initialize IPC wrapper (spawns subprocess)
        self._env = HabitatIPCWrapper(ipc_config)

        if objects is not None:
            for obj in objects:
                obj_dict = asdict(obj) if is_dataclass_instance(obj) else obj
                pb_add_object_request = protocol_pb2.AddObjectRequest()
                pb_add_object_request.name = obj_dict["name"]
                ...
                if "semantic_id" in obj_dict:
                    semantic_id = obj_dict["semantic_id"]
                    pb_add_object_request.semantic_id = semantic_id
                if "primary_target_object" in obj_dict:
                    primary_target_object = obj_dict["primary_target_object"]
                    pb_add_object_request.primary_target_object = primary_target_object

                self._env.add_object(
                    name=pb_add_object_request.name,
                    position=(
                        pb_add_object_request.position.x,
                        pb_add_object_request.position.y,
                        pb_add_object_request.position.z,
                    ),
                    rotation=(
                        pb_add_object_request.rotation.w,
                        pb_add_object_request.rotation.x,
                        pb_add_object_request.rotation.y,
                        pb_add_object_request.rotation.z,
                    ),
                    scale=(
                        pb_add_object_request.scale.x,
                        pb_add_object_request.scale.y,
                        pb_add_object_request.scale.z,
                    ),
                    semantic_id=pb_add_object_request.semantic_id if pb_add_object_request.HasField("semantic_id") else None,
                    primary_target_object=pb_add_object_request.primary_target_object if pb_add_object_request.HasField("primary_target_object") else None,
                )
...
ObjectConfig = None  # Not used in IPC version
SingleSensorAgentArgs = None  # Not used in IPC version  
MultiSensorAgentArgs = None  # Not used in IPC version
